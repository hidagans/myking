...
if(!users) return await conn.reply(m.chat, `${fg}pengguna tidak ditemukan, pengguna harus dalam grup ini`, fkontak,  m)
...
if(user === m.sender) return await conn.reply(m.chat, `${fg}Kamu tidak bisa menjadi pasangan dengan diri sendiri`, fkontak,  m)
...
return await conn.reply(m.chat, `Anda tidak dapat menerima jika tidak ada yang menyatakan diri, nyatakan diri Anda dengan *{tu}* untuk mengatakan apakah mereka menerima atau menolak Anda.`, fkontak,  m)
...
handler.command = /^(terima|accept)$/i
handler.group = true
