...
const messageText = `⚠️ WARNING ⚠️\nAviso (${messageNumber}/3) ${user.bannedReason ? `\n*Motivo:* ${user.bannedReason}` : 'Alasan: (spam)'}
*👉🏻 Anda dapat menghubungi pemilik Bot jika Anda merasa hal tersebut merupakan kesalahan atau untuk mendiskusikan pemblokiran Anda.*

👉 wa.me/6285870345387`.trim();
m.reply(messageText);
...
if (remainingTime > 0) {
const messageText = `*Tunggu ${remainingTime} sebelum menggunakan perintah lagi *`;
m.reply(messageText);
return;
} else {
user.commandCount = 0;
...
text: `🫥 Tidak memiliki MajestyCoins`, 
contextInfo:{
...
conn.sendMessage(m.chat, { 
text: `${lenguajeGB['smsCont9']()} *${plugin.level}* ${lenguajeGB['smsCont10']()} *${_user.level}* ${lenguajeGB['smsCont11']()} *${usedPrefix}level*`, 
contextInfo:{
forwardingScore: 9999999,
isForwarded: true, 
mentionedJid:[m.sender],
...
if (!m.fromMem && m.text.match(/(@6285870345387|majestyBOT|Bot|MajestyBot|majestybot|majesty|botmajesty|yearigans|:v)/gi)) {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*MajestyBot*') :
...
await this.reply(msg.chat, `[ ANTI DELETE ]\n\n*Pengguna @${participant.split`@`[0]} Menghapus pesan,* \n*Mengirim pesan...*\n\n*Untuk menonaktifkan fungsi ini ketik:*
#disable antidelete
