...
if (existsSync(dir)) conn.logger.info(` TELAH DI PERBARUI - '${filename}' BERHASIL`);
else {
conn.logger.warn(`FILE TELAH DIHAPUS : '${filename}'`);
return delete global.plugins[filename];
}
} else conn.logger.info(`PLUGIN BARU TERDETEKSI : '${filename}'`);
const err = syntaxerror(readFileSync(dir), filename, {
sourceType: 'module',
allowAwaitOutsideFunction: true,
});
if (err) conn.logger.error(`SE MENDETEKSI KESALAHAN SINTAKSIS | KESALAHAN SINTAKSIS SAAT MEMUAT '${filename}'\n${format(err)}`);
else {
try {
const module = (await import(`${global.__filename(dir)}?update=${Date.now()}`));
global.plugins[filename] = module.default || module;
} catch (e) {
conn.logger.error(`ADA BUG YANG MEMBUTUHKAN PLUGIN '${filename}\n${format(e)}'`);
} finally {
